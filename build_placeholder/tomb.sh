...
VERSION=2.3
SRCNAM=Tomb-${VERSION}.tar.gz
WRKDIR=Tomb-${VERSION}
EXTNAM=Tomb
TMPDIR=/tmp/Tomb
URL="https://files.dyne.org/tomb/${SRCNAM}"

######################################################
# Prepare extension creation                         #
######################################################

deps="libgcrypt-dev libnotify-dev gtk2-dev squashfs-tools compiletc"
...
make
make install DESTDIR=$TMPDIR
cp tomb-gtk-tray $TMPDIR/usr/local/bin

# Install GTOMB
cd ../gtomb
cp -a gtomb $TMPDIR/usr/local/bin
...
cat > $TMPDIR/usr/local/tce.installed/Tomb << EOF
#!/bin/sh

GPGBIN="/usr/local/bin/gpg"
GPG2BIN="/usr/local/bin/gpg2"
...
chmod 755 $TMPDIR/usr/local/tce.installed/Tomb
chown tc:staff $TMPDIR/usr/local/tce.installed/Tomb

# Move files to doc extension

mkdir -p $TMPDIR-doc/usr/local/share
mv $TMPDIR/usr/local/share/man $TMPDIR-doc/usr/local/man
